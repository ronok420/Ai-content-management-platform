...
export const updateMyProfileService = async (userId, updateData) => {
  // Prevent users from updating their role or other protected fields
...
/**
 * Gets all content published by a specific user.
 * @param {string} userId - The ID of the user whose content to fetch.
 * @returns {array} A list of the user's published content.
 */
export const getUserContentService = async (userId) => {
  const userContent = await Content.find({ author: userId, status: 'published' })
    .sort({ createdAt: -1 })
