...
/**
 * A flexible middleware to check if a user has one of the allowed roles.
 * @param {array} allowedRoles - An array of role strings (e.g., ['creator', 'admin']).
 * @returns {function} The middleware function.
 */
export const checkRole = (allowedRoles) => {
  return (req, res, next) => {
    if (!req.user || !req.user.role) {
      return generateResponse(res, 401, false, 'Authentication error: User role not found.');
    }

    const { role } = req.user;

    if (!allowedRoles.includes(role)) {
      return generateResponse(res, 403, false, `Forbidden: Access is restricted to the following roles: ${allowedRoles.join(', ')}.`);
    }
